...
Keywords Only Version with OTP Authentication
"""
...
class GmailPhishingAnalyzer:
    ...
    def analyze_email(self, email_data):
        ...
        # 1️⃣ Check for keywords in subject
        subject_score = 0
        ...
        # 2️⃣ Check for keywords in body
        body_score = 0
        body_keywords_found = []
        for keyword in self.phishing_keywords:
        ...
        for keyword in self.suspicious_name_keywords:
            if keyword in display_name and len(display_name) > 0:
        ...
        if ('gmail' in sender_domain or 'google' in sender_domain) and \
        ...
        if is_impersonator:
            ...
            if is_suspicious_display or phishing_score >= 40:
                # Suspicious display name OR high keyword score
            ...
            # Other whitelisted domains - cap at 30
            phishing_score = min(phishing_score, 30)
        ...
        return {
            ...
            'details': {
                ...
                'keywords_found': {
                    'subject': subject_keywords_found,
                    'body': body_keywords_found[:10]
                }
            },
...
def gmail_demo_scan():
    ...
    sample_emails = [
        ...
        },
        {
            ...
            'from': 'security <zbnm92260@gmail.com>',
            'date': datetime.now().strftime('%a, %d %b %Y %H:%M:%S %z'),
        ...
        },
        {
            ...
            'from': 'Netflix <account@netflix-billing.net>',
            'date': datetime.now().strftime('%a, %d %b %Y %H:%M:%S %z'),
            'body_text': 'Dear Customer, Your Netflix account will be suspended due to payment failure. Update your payment method: http://netflix-account-update.net/payment If not updated within 24 hours, your account will be terminated. Netflix Billing Team',
            'body_html': '',
            'attachments': []
        },
        {
            ...
            'date': datetime.now().strftime('%a, %d %b %Y %H:%M:%S %z'),
            'body_text': 'Dear PayPal User, We detected unusual activity on your PayPal account. A transaction of $499.99 was attempted from an unknown device. If this was not you, please secure your account: http://paypal-security-verify.net/login If ignored, your account will be limited. PayPal Security Center',
            'body_html': '',
            'attachments': []
        },
        {
            'uid': 'gmail_demo5',
            'subject': 'LinkedIn Job Alert: 15 New Jobs',
            'from': 'LinkedIn <jobalerts-noreply@linkedin.com>',
            'date': datetime.now().strftime('%a, %d %b %Y %H:%M:%S %z'),
            'body_text': 'Hi, We found 15 new jobs matching your profile. View jobs: https://www.linkedin.com/jobs/view Happy job hunting! The LinkedIn Team',
            'body_html': '',
            'attachments': []
        }
    ]
...
def monitor_gmail_loop(gmail_config, user_email):
    ...
    print(f"🔍 Using keywords-only detection")
            ...
            for email_data in emails:
                ...
                if analysis['is_phishing']:
                    ...
                    socketio.emit('gmail_phishing_alert', {
                        ...
                        'message': '🚨 GMAIL PHISHING DETECTED!',
                        'is_gmail': True,
                        'has_attachments': len(email_data.get('attachments', [])) > 0
                    })
